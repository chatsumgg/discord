...
<template>
  ...
  <div
    class="relative pl-[72px] pr-[48px] py-[1px] before:absolute before:left-0 font-discord transition-colors dark:font-normal font-medium"
    :class="[
      waitingCommand ? 'opacity-70 pointer-events-none' : '',
      hideAuthor || !author ? '' : 'mt-[17px]',
      mention ? 'dark:bg-[#484138] dark:before:bg-[#f0b131] bg-[#fdf7ed] before:bg-[#906210] hover:bg-[#fcf0dc] before:h-full before:w-[3px]' : '',
      (!waitingCommand && ephemeral) ? 'dark:bg-[#333644] bg-[#f7f8fe] dark:hover:bg-[#36394d] hover:bg-[#eff1fd] before:bg-[#5965f2]' : 'dark:hover:bg-[#2d2f35] hover:bg-[#f8f8f8]',
      ephemeral ? 'before:h-full before:w-[3px]' : ''
    ]">


    <div v-if="ephemeral" class="ephemereal relative text-sm dark:text-[#b5bac1] text-[#4e5058] mb-1">
      <span class="dark:font-medium font-semibold dark:text-white text-black">
        <img
      ...
      used
      <span class="font-medium hover:underline cursor-pointer dark:text-[#00ACF8] text-[#006ce7ab]">{{ commandUsed }}</span>
    </div>
    ...
    <div class="flex flex-col items-baseline">
      ...
      <h3
        ...
        <span class="dark:text-[#f2f3f5] text-black leading-[22px] dark:font-medium font-semibold mr-1 hover:underline underline-offset-2 cursor-pointer">{{ author }}</span>
        ...
        <span class="text-xs font-medium dark:text-[#949ba4] text-[#5c5e66] ml-1">{{ date }}</span>
      </h3>

      <div class="-mt-px leading-[22px] dark:text-[#DBDEE1] text-[#313338]">
      ...
      <div v-if="ephemeral">
        <div v-if="!waitingCommand" class="inline-flex items-center text-xs font-medium h-full mt-1">
          ...
          <span class="dark:text-[#949ba4] text-[#5C5E66] mr-1">Only you can see this • </span>

          <span class="dark:text-[#00a8fc] text-[#006ce7] hover:underline cursor-pointer">Dismiss message</span>
